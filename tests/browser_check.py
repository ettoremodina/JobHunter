    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        try:
            with sync_playwright() as p:
                ...
                expect(page.locator("#queue-items button").filter(has_text="Energy Lab")).to_have_count(1)
                page.get_by_role("button", name="Prepara approfondimento in chat", exact=True).click()
                ...
                expect(page.get_by_role("button", name="Aggiorna", exact=True)).to_have_count(4)
                assert page.get_by_role("button", name="Aggiorna", exact=True).last.is_disabled()
