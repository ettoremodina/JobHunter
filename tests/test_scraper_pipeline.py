"""Verify acquisition, description recovery and selection together without network access."""
...
class ScraperPipelineTests(unittest.TestCase):
    ...
    def test_bounded_and_full_collection_recover_before_selection(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / 'config/sweep.json').write_text(json.dumps({'description_followup_all': True}))
            ...
            html = '<div class="show-more-less-html__markup"><p>Minimum 5 years of relevant experience required.</p></div>'
            with closing(Archive(root / 'archive.db')) as archive:
                ...
                self.assertEqual(report['description_followup']['remaining_missing'], 0)
                self.assertEqual(report['analytics']['health']['with_description'], 1)
                self.assertEqual(report['queue']['items'], [])
                self.assertEqual(report['analytics']['selection'], [{'label': 'excluded', 'count': 2}])
                with patch('jobhunter.sweep.ROOT', root), patch('jobhunter.descriptions.ROOT', root), patch('jobhunter.sweep.airtable_rows', side_effect=AssertionError('Listings must not be fetched again')), patch('jobhunter.descriptions.fetch', side_effect=AssertionError('Fresh cached text must be reused')):
                    resumed = sweep(archive, cfg, resume_after_listings=True)
                resumed_report = json.loads(Path(resumed['report']).read_text(encoding='utf-8'))
                self.assertTrue(resumed_report['resumed_after_listings'])
                self.assertEqual(resumed_report['analytics']['selection'], report['analytics']['selection'])
                self.assertEqual(resumed_report['sources'], {})
