"""Controlli gerarchici sull'archivio, in sola lettura.

La dashboard naviga ambito, sezione, controllo e sottogruppo. Ogni lente resta una domanda
sola («quali aziende non hanno una descrizione?»), con conteggio e paginazione. Aggiungere
una lente significa aggiungere una voce completa a LENSES, non una funzione nuova ovunque.
"""
...
# `scope` e `section` formano il percorso mostrato dalla dashboard. Il campo `facet`
# divide poi una lente in gruppi (il motivo dello scarto, lo stato del recupero):
# senza, «annunci scartati» sarebbe un elenco di tredicimila righe senza una domanda dentro.
LENSES = {
    'aziende-senza-descrizione': {
        'label': 'Aziende senza descrizione', 'unit': 'aziende', 'scope': 'aziende', 'section': 'Dati mancanti',
    ...
    'annunci-senza-descrizione': {
        'label': 'Annunci senza descrizione', 'unit': 'annunci', 'scope': 'annunci', 'section': 'Dati mancanti',
    ...
    'annunci-scartati-dal-regex': {
        'label': 'Annunci scartati dal regex, per motivo', 'unit': 'annunci', 'scope': 'annunci',
        'section': 'Esiti e motivi', 'facet_label': 'Motivo',
    ...
    'ruoli-buoni-azienda-cieca': {
        ...
        'scope': 'aziende', 'section': 'Dati mancanti',
    ...
    'aziende-non-categorizzate': {
        'label': 'Aziende senza categoria', 'unit': 'aziende', 'scope': 'aziende', 'section': 'Dati mancanti',
        'sql': """SELECT c.id, c.id company_id, c.name, '' facet,
    ...
    'aziende-decise-a-mano': {
        ...
        'section': 'Decisioni manuali', 'facet_label': 'Decisione',
    ...
    'recupero-descrizione-fallito': {
        'label': 'Recupero della descrizione non riuscito, per esito', 'unit': 'annunci', 'scope': 'annunci',
        'section': 'Recupero dati', 'facet_label': 'Esito',
    ...
    'annunci-per-lingua': {
        'label': 'Annunci per lingua dell’originale', 'unit': 'annunci', 'scope': 'annunci',
        'section': 'Lingue e località', 'facet_label': 'Lingua',
    ...
    'annunci-in-lingua-sconosciuta': {
        'label': 'Annunci in una lingua che non conosci', 'unit': 'annunci', 'scope': 'annunci',
        'section': 'Lingue e località', 'facet_label': 'Lingua',
        'sql': """SELECT o.id, o.company_id, c.name, json_extract(e.decision,'$.requirements.written_in.name') facet,
                         json_extract(o.data,'$.title') detail, json_extract(o.data,'$.application_url') url
                  FROM search_eligibility e JOIN opportunities o ON o.id=e.opportunity_id
                  ...
                  ORDER BY c.name COLLATE NOCASE"""},
    'localita-da-mappare': {
        'label': 'Località senza città riconosciuta', 'unit': 'annunci', 'scope': 'annunci',
        'section': 'Lingue e località', 'facet_label': 'Località',
...
def lenses(archive):
    ...
    for key, lens in LENSES.items():
        ...
        result.append({'id': key, 'label': lens['label'], 'unit': lens['unit'],
                       'scope': lens['scope'], 'section': lens['section'],
...
def rows(archive, key, value='', offset=0, limit=50, filter_value=False):
    """Una pagina della lente, ristretta al gruppo scelto anche quando il valore è vuoto."""
    if key not in LENSES:
        raise ValueError('Lente sconosciuta')
    prepare(archive)
    lens = LENSES[key]
    filtered = filter_value or bool(value)
    where = ' WHERE facet=?' if filtered else ''
    arguments = ([value] if filtered else []) + [min(max(int(limit), 1), 200), max(int(offset), 0)]
    total = archive.db.execute(f"SELECT count(*) FROM ({lens['sql']})" + where,
                               [value] if filtered else []).fetchone()[0]
    items = archive.db.execute(f"SELECT * FROM ({lens['sql']})" + where + ' LIMIT ? OFFSET ?', arguments).fetchall()
    return {'id': key, 'label': lens['label'], 'unit': lens['unit'], 'scope': lens['scope'],
            'section': lens['section'], 'facet_label': lens.get('facet_label', ''),
